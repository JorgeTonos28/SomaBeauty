<x-app-layout>
    ...
    <div x-data="ticketForm()" class="py-6 max-w-4xl mx-auto sm:px-6 lg:px-8">
        <form x-ref="form" action="{{ route('tickets.update', $ticket) }}" method="POST" @submit.prevent="submitForm" class="space-y-6 pb-32">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="vehicle_type_id" required class="form-select w-full mt-1">
                    ...
                    @foreach ($vehicleTypes as $type)
                        <option value="{{ $type->id }}" @selected($ticket->vehicle_type_id == $type->id)>{{ $type->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="washer_id" required class="form-select w-full mt-1">
                    ...
                    @foreach ($washers as $washer)
                        <option value="{{ $washer->id }}" @selected($ticket->washer_id == $washer->id)>{{ $washer->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                @foreach ($services as $service)
                    <div class="flex items-center space-x-2 mt-1">
                        <input type="checkbox" name="service_ids[]" value="{{ $service->id }}" @checked(in_array($service->id, $selectedServices))>
                        ...
                        <div class="flex gap-4 mb-2 items-center">
                            <select name="product_ids[]" class="form-select w-full" onchange="updateTotal()">
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" @selected($p['product_id']==$product->id)>{{ $product->name }} (RD$ {{ number_format($product->price, 2) }})</option>
                                @endforeach
                            </select>
                            <input type="number" name="quantities[]" placeholder="Cantidad" min="1" class="form-input w-24" value="{{ $p['quantity'] }}" oninput="updateTotal()">
            ...
            <div>
                ...
                <input type="number" name="paid_amount" id="paid_amount" required step="0.01" class="form-input w-full mt-1" value="{{ $ticket->paid_amount }}" oninput="updateChange()">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Método de Pago</label>
                <select name="payment_method" required class="form-select w-full mt-1">
                    <option value="efectivo" @selected($ticket->payment_method=='efectivo')>Efectivo</option>
                    <option value="tarjeta" @selected($ticket->payment_method=='tarjeta')>Tarjeta</option>
                    <option value="transferencia" @selected($ticket->payment_method=='transferencia')>Transferencia</option>
                    <option value="mixto" @selected($ticket->payment_method=='mixto')>Mixto</option>
                </select>
            </div>
